...
class AlphaZeroConfig:
    ...
    def __init__(self):
        ...
        # 神经网络参数
        self.NUM_CHANNELS = 256  # 卷积层通道数（恢复为完整大小）
        self.NUM_RESIDUAL_BLOCKS = 20  # 残差块数量（恢复为完整大小）
        self.DROPOUT_RATE = 0.3  # Dropout比率
        self.BATCH_NORM_MOMENTUM = 0.9  # 批归一化动量
        
        # MCTS参数
        self.NUM_MCTS_SIMS = 400  # 每步MCTS模拟次数（适中值以平衡速度和性能）
        self.C_PUCT = 1.0  # PUCT公式中的探索常数
        self.DIRICHLET_ALPHA = 0.3  # Dirichlet噪声参数
        self.DIRICHLET_EPSILON = 0.25  # Dirichlet噪声权重
        
        # 自我对弈参数
        self.SELF_PLAY_GAMES = 100  # 每次迭代的自我对弈局数（恢复为完整数量）
        self.PARALLEL_GAMES = 20  # 并行自我对弈的游戏数（增加以利用多GPU）
        ...
        self.NUM_WORKERS = 8  # 数据加载器的工作进程数（增加以提高数据加载速度）
